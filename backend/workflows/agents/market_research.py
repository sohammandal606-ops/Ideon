...
async def market_research_node(state: StartupState) -> dict:
    """
    Conducts market research for the startup idea.
    """
    llm = get_llm()
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                ...
                "Analyze the target audience and market size for a new startup.",
            ),
            (
                "user",
                ...
                "Please provide structured market research including audience, size, and trends.",
            ),
        ]
    )
